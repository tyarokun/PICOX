...
PICOXは、坂井弘亮氏が開発した学習用組込みOS「KOZOS」を基に、
Raspberry Pi Pico（RP2040 / Arm Cortex-M0+）向けに作成した組込みOSです。

PICOXとして新たに作成した部分およびKOZOSを基にした部分には、
川合堂ライセンス-01（KL-01）を適用します。

# KOZOS
PICOXは、坂井弘亮氏が開発した学習用組込みOS「KOZOS」を基に作成しています。

各ファイルまたは各部分に別の記載がない限り、PICOXとして新たに作成した部分と
KOZOSを基にした部分には、川合堂ライセンス-01（KL-01）を適用します。

元となったプロジェクト：
  プロジェクト名：KOZOS
  作者：坂井弘亮
  公式サイト：https://kozos.jp/kozos/
  ライセンス：川合堂ライセンス-01（KL-01）

# 参考にしたプロジェクト

PICOXのSDカード、SPIおよびFAT32ファイルシステムに関連する実装では、
以下のオープンソースプロジェクトを参考資料として参照しました。

プロジェクト名：no-OS-FatFS-SD-SPI-RPi-Pico
作者・リポジトリ所有者：carlk3
リポジトリ：
https://github.com/carlk3/no-OS-FatFS-SD-SPI-RPi-Pico
ライセンス：Apache License 2.0

同プロジェクトは、Raspberry Pi PicoでSPI接続のSDカードを使用するための
実装例として参照しました。

PICOXのSDカード、SPIおよびFAT32関連のコードは、PICOX向けに作成したものです。
同プロジェクトのソースコードは、PICOXのリポジトリには含まれていません。

# 川合堂ライセンス-01

以下に、川合堂ライセンス-01の原文を掲載します。
